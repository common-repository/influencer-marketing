 ...
 */
if (!function_exists('imkt_dashboard_callback')) {

    function imkt_dashboard_callback() {

        include_once( ABSPATH . WPINC . '/feed.php' );
        wp_enqueue_script('swift-form-jstz', IMKT__PLUGIN_URL . 'admin/js/jstz.min.js', '', '', true);
        wp_enqueue_style('swift-dashboard', IMKT__PLUGIN_URL . 'admin/css/swift-dashboard.css', '', '', '');
        wp_enqueue_script(IMKT_PLUGIN_PREFIX . 'dashboard-script', IMKT__PLUGIN_URL . 'admin/js/swift-dashboard.js', array('jquery'), '', true);
        wp_enqueue_style('swiftcloud-fontawesome', IMKT__PLUGIN_URL . 'public/css/font-awesome.min.css', '', '', '');

        $subscribed = get_option(IMKT_PLUGIN_PREFIX . 'dashboard_subscribe');
        $subs_checkbox_flag = isset($_COOKIE[IMKT_PLUGIN_PREFIX . 'dashboard_unsubscribe']) && $_COOKIE[IMKT_PLUGIN_PREFIX . 'dashboard_unsubscribe'] == 1 ? "block" : "none";
        ...
        <div class="wrap dashboard-wrap">
            ...
            <!--subscribe box-->
            <div class="dashboard-subscribe-block" id="imkt_subscribe" style="display: <?php echo $subs_box_flag; ?>;">
                ...
                <form method="post" id="frm_imkt_dashboard_subscribe">
                    ...
                    <input type="hidden" name="formid" value="647" id="formid" />
                    ...
                    <div class="dashboard-col-8 col-left">
                        <div class="col-dashboard-block">
                            ...
                            <div class="col-dashboard-block-content">
                                <?php
                                $rss = fetch_feed('https://SwiftCRM.Com/support/tag/impress/feed');
                                $maxitems = 0;
                                ...
                                <?php if ($maxitems == 0) : ?>
                                    <div class="col-dashboard-item">
                                        <p><?php _e('No items', 'impress-pr'); ?></p>
                                    </div>
                                <?php else : ?>
 ...
 *      cookie value : 0 || 1
 */
add_action('wp_ajax_' . IMKT_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox', IMKT_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox_callback');
add_action('wp_ajax_nopriv_' . IMKT_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox', IMKT_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox_callback');
if (!function_exists(IMKT_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox_callback')) {

    function imkt_dashboard_subscribe_checkbox_callback() {
        if (isset($_POST['action']) && !empty($_POST['action']) && $_POST['action'] == IMKT_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox') {
            setcookie(IMKT_PLUGIN_PREFIX . 'dashboard_unsubscribe', "", time() - 1, "/", '');
            echo "1";
        }
        wp_die();
    }
...
add_action('wp_ajax_' . IMKT_PLUGIN_PREFIX . 'dashboard_close_subscribe', IMKT_PLUGIN_PREFIX . 'dashboard_close_subscribe_callback');
add_action('wp_ajax_nopriv_' . IMKT_PLUGIN_PREFIX . 'dashboard_close_subscribe', IMKT_PLUGIN_PREFIX . 'dashboard_close_subscribe_callback');
if (!function_exists(IMKT_PLUGIN_PREFIX . 'dashboard_close_subscribe_callback')) {

    function imkt_dashboard_close_subscribe_callback() {
        if (isset($_POST['action']) && !empty($_POST['action']) && $_POST['action'] == IMKT_PLUGIN_PREFIX . 'dashboard_close_subscribe') {
            setcookie(IMKT_PLUGIN_PREFIX . 'dashboard_unsubscribe', "1", time() + (10 * 365 * 24 * 60 * 60), "/", '');
            echo "1";
        }
        wp_die();
    }
...
add_action('wp_ajax_' . IMKT_PLUGIN_PREFIX . 'dashboard_subscribe', IMKT_PLUGIN_PREFIX . 'dashboard_subscribe_callback');
add_action('wp_ajax_nopriv_' . IMKT_PLUGIN_PREFIX . 'dashboard_subscribe', IMKT_PLUGIN_PREFIX . 'dashboard_subscribe_callback');

if (!function_exists(IMKT_PLUGIN_PREFIX . 'dashboard_subscribe_callback')) {

    function imkt_dashboard_subscribe_callback() {
        check_ajax_referer('swiftdashboard_subs_form', 'swiftdashboard_subs_form');
        if (isset($_POST['action']) && !empty($_POST['action']) && $_POST['action'] == IMKT_PLUGIN_PREFIX . 'dashboard_subscribe') {
            parse_str($_POST['data'], $subscribe_form_data);

            update_option(IMKT_PLUGIN_PREFIX . 'dashboard_subscribe', true);
            setcookie(IMKT_PLUGIN_PREFIX . 'dashboard_unsubscribe', "", time() - 1, "/", '');
