...
jQuery(document).ready(function($) {
    ...
    jQuery(".dashboard-subscribe").on("click", function(e) {
        ...
        if (jQuery.trim(jQuery("#email").val()) === '') {
            jQuery("#frm_imkt_dashboard_subscribe").after('<span class="ssign-error" style="color:#fff;margin-bottom:10px;">Email is required.</span>');
            error++;
        } else if (!ValidateEmail(jQuery.trim(jQuery("#email").val()))) {
            jQuery("#frm_imkt_dashboard_subscribe").after('<span class="ssign-error" style="color:#fff;margin-bottom:10px;">Invalid email.</span>');
            error++;
        }
        ...
        if (error > 0) {
            ...
            var data = {
                'action': imkt_pluginPrefix + 'dashboard_subscribe',
                'data': jQuery("#frm_imkt_dashboard_subscribe").serialize(),
                'swiftdashboard_subs_form': jQuery('#swiftdashboard_subs_form').val()
            };
            jQuery.post(ajaxurl, data, function(response) {
                if (response == 1) {
                    jQuery(".dashboard-subscribe-block").fadeOut();
                    jQuery("#imkt_subscribe").after('<div id="message" class="notice notice-success is-dismissible"><p>Thanks! Welcome to the club.</p></div>');
                }
            });
        }
    });
    ...
    jQuery(".dashboard-close-subscribe-block").on("click", function() {
        var data = {
            'action': imkt_pluginPrefix + 'dashboard_close_subscribe',
        };
        jQuery.post(ajaxurl, data, function(response) {
            if (response == 1) {
    ...
    jQuery("#swift_dashboard_subscribe").on("click", function() {
        ...
        if (check == 1) {
            var data = {
                'action': imkt_pluginPrefix + 'dashboard_subscribe_checkbox',
            };
            jQuery.post(ajaxurl, data, function(response) {
                if (response == 1) {
